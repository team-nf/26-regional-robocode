...
public class AimAndShootCommand extends Command {
  ...
  public void execute() {
    ...
    robotAngleToHub = Math.atan2(hubAimPose.getY() - robotPose.getY(), hubAimPose.getX() - robotPose.getX());
    angleError = robotAngleToHub - robotPose.getRotation().getRadians();
    angleError = Math.atan2(Math.sin(angleError), Math.cos(angleError));

    if(DriverStation.getAlliance().get() == Alliance.Red) robotAngleToHub += Math.PI;
